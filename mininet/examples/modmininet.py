...
class Npairs:
    ...
    def __init__(self):
        ...
        log.close()
        log=open(self.servers,"w")
        log.close()

    def logfull(self):
        log = open(self.filename,"a")
        out = self.sname + "," + str(self.sport) + "," + self.smac + "," + self.sIP + ","
        out = out + self.hname + "," + str(self.hport) + "," + self.hmac + "," + self.hIP
        out = out + "\n"
        log.write(out)
        log.close()

    def setMac(self,sw,intname,h):
        r = sw.cmd("ifconfig","|","grep",intname,"|","awk","\'{print $5}\'","|","tr","-d","\'\n\'")
        #r = sw.cmd("awk","\'\{print $5\}\'",r)
        if not r:
            r = sw.MAC()
        if h == 1:
            self.hmac = r
    ...
    def start( self ):
        ...
        if self.net is None:
            ...
            for sw in n.switches:
                lnk.sname = sw.name
                for h in n.hosts + n.switches:
                    intf = sw.connectionsTo(h)
                    if intf:
                        p = intf[0][0]
                        lnk.sport = sw.ports[p]
                        lnk.setMac(sw,p.name,0)
                        lnk.sIP = sw.IP(p)
                        if not lnk.sIP:
                            lnk.sIP = sw.IP()
                        p = intf[0][1]
